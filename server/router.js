...
function replaceStr(str) {
  ...
  for (let key of abcds) {
    str = str.replace(key + '．', '').replace(key + '、', '').replace(key + '.', '')
  }
  return str.trim()
}

async function initExercise(context) {
  const filePath = path.join(__dirname, './excels')
  const files = fs.readdirSync(filePath)
  for (let file of files) {
    if (file === 'index.xlsx') continue
    let RedCellDatas = xlsx.parse(`${__dirname}/excels/${file}`)[0].data
    ...
    if (subject) {
      Chapter.collection.findOneAndUpdate({ name: chapterName }, { name: chapterName, num: chapterNum, subjectId: subject._id, createdAt: Date.now(), updatedAt: Date.now() }, { upsert: true })
      let chapter = await Chapter.collection.findOne({ name: chapterName })
      if (chapter) {
        Section.collection.findOneAndUpdate({ name: sectionName }, { name: sectionName, num: sectionNum, chapterId: chapter._id, createdAt: Date.now(), updatedAt: Date.now() }, { upsert: true })
